...
## 27. Uso racional de antimicrobianos (DOT) + Farmacêutico igualado ao Coordenador (2026-08-19)

Pacote grande, duas frentes pedidas juntas pelo cliente.

**Vigilância de antimicrobianos (DOT — Days of Therapy):**
- `Medicamento.e_antimicrobiano` (bool, migration `0006_farma_igual_coordenacao`) — curado manualmente no cadastro (checkbox em Medicamentos), não existe auto-detecção. Pensado pra deixar aberto: outras classes (ex. controlados) podem ganhar um campo irmão no futuro sem precisar generalizar isso agora.
- Saída de item `e_antimicrobiano=true` passa a EXIGIR paciente/prontuário (checado em `SaidaService`, não só na UI) — sem isso não dá pra contar dias por paciente. Nos demais itens continua opcional, como já era.
- `GET /relatorios/antimicrobianos` (Farmacêutico/Coordenador — mesmo grupo que já vê dado de paciente): agrupa Saídas de antimicrobiano por paciente+medicamento, conta dias-calendário consecutivos de dispensação terminando na mais recente, só lista quem tem dispensação nos últimos 2 dias (uso corrente, não histórico) e mais de `dias_minimo` (default 7) dias seguidos. **Limitação assumida de propósito**: é aproximação a partir de dispensação da farmácia, não confirmação real de administração à beira do leito — o hospital não tem eMAR, mesma limitação que já existia no resto do sistema. Optamos por DOT (mais simples) em vez de DDD (dose diária definida da OMS, métrica populacional) — decisão do cliente, DDD ficou fora de escopo.
- Nova aba "Antimicrobianos" em Relatórios, mostra paciente/prontuário/medicamento/dias/doses individuais. Sem exportação PDF/Excel ainda (não construído especulativamente).
- Testado: criado medicamento antimicrobiano de teste, saída sem paciente rejeitada (400), com paciente aceita; 7 dispensações retroativas inseridas direto no banco pra simular 8 dias seguidos (não dá pra "voltar o relógio" via API) — relatório calculou `dias_consecutivos=8` corretamente, confirmado na tela.

**Farmacêutico = Coordenador em nível de acesso** (pedido explícito do cliente — "Farma" ganha praticamente tudo que Coordenador tinha a mais):
- **Descarte deixou de ser fluxo de 2 etapas** (Farmacêutico solicitava, Coordenador aprovava/rejeitava) — agora é ação direta, `POST /descartes` só, igual Entrada/Saída/Ajuste. Farmacêutico e Coordenador têm o mesmo acesso. Registros antigos com `status=pendente_aprovacao` continuam intactos no banco (histórico não foi tocado), só não existe mais rota pra aprovar/rejeitar — ficam congelados como estavam.
- **Ajuste de Estoque** aberto ao Farmacêutico (era exclusivo do Coordenador desde a seção 24, ~5 dias atrás — mudança explícita pedida pelo cliente nesta mesma conversa).
- **Consolidado de todas as unidades** (relatórios financeiros + `GET /lotes` sem filtro, inclusive a landing pós-login) aberto ao Farmacêutico — `resolver_unidade_escopo` (app/api/deps.py) passou a tratar Farmacêutico igual Coordenador. Efeito prático: a tela Estoque atual do Farmacêutico agora mostra "todas as unidades" por padrão, igual já era pro Coordenador.
- **Trilha de Auditoria continua exclusiva do Coordenador** — decisão explícita do cliente, não entrou na equalização.
- **Reposição/Devolução de carrinho** abertas também ao Coordenador (eram exclusivas do Farmacêutico) — "coordenador deve ter plenos poderes", pedido explícito.
- **Em troca da autorização prévia de Descarte que deixou de existir**: `GET /relatorios/atividade-recente` (exclusivo Coordenador) — descartes, ajustes e saídas de empréstimo/doação dos últimos N dias (default 7), com quem fez (usuário já gravado desde sempre em `usuario_id`, isto só expõe de forma direta). Vira o 3º bloco do popup de login (seção 25), visível só pro Coordenador — supervisão continua existindo, só que depois do fato em vez de travando antes.
